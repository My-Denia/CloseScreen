...
buildNpmPackage {
  ...
  npmDepsHash = "sha256-dSb0pqsrYIE+PCquudDNVhXolWwg7/ZBAIHRMfj2Nnc=";
  ...
  installPhase = ''
    ...
    mkdir -p "$out/lib/closescreen/public"
    cp -r public/wallpapers "$out/lib/closescreen/public/wallpapers"

    # Wrap system electron with the app directory
    mkdir -p "$out/bin"
    makeWrapper "${electron}/bin/electron" "$out/bin/closescreen" \
      --add-flags "$out/lib/closescreen" \
    ...
    for size in 16 24 32 48 64 128 256 512 1024; do
      ...
      if [ -f "$icon" ]; then
        install -Dm644 "$icon" \
          "$out/share/icons/hicolor/''${size}x''${size}/apps/closescreen.png"
      fi
    done
