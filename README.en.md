...
- Release line: this fork versions independently as `v1.5.0-fork.*`. `v1.5.0-fork.4` is the current
  release.
...
The defining change is the Windows capture chain, which was migrated to Rust and is now the default:

- `parseWindowsNativeBackend` in `electron/native-bridge/windowsNativeHelpers.ts` returns `rust` when
  `CLOSESCREEN_WINDOWS_CAPTURE_BACKEND` is unset, so an unconfigured install records through the Rust
  WGC/WASAPI/Media Foundation helpers.
- The legacy C++ helpers stay in the package as an explicit rollback: set
  `CLOSESCREEN_WINDOWS_CAPTURE_BACKEND=legacy` before launching CloseScreen. The value must be exactly
  `rust` or `legacy`; any other value is a configuration error rather than a silent fallback, and a
  recording does not switch backend after it starts.
- The cutover landed in commit `71b991a` (PR #82 in this repository) and first shipped publicly in
  `v1.5.0-fork.4`.
- The Rust workspace pins toolchain `1.96.1` in `electron/native/rust/rust-toolchain.toml` and
  generates [RUST_THIRD_PARTY_NOTICES.md](./RUST_THIRD_PARTY_NOTICES.md).

Other work carried by this fork rather than inherited from upstream:

- Export-path and `read-binary-file` IPC hardening in the main process.
- Manual, plan-first recording cleanup restricted to session artifacts referenced by session manifests.
- An update check pinned to this fork's own repository, link-only and without an updater service.
- A release workflow that verifies the tag and source SHA and publishes a flat asset set with SHA-256
  checksums.
- A documentation set covering the [support matrix](./docs/SUPPORT_MATRIX.md),
  [privacy notes](./docs/PRIVACY.md), the [regression checklist](./docs/REGRESSION_CHECKLIST.md), and
  the [Windows native recorder roadmap](./docs/engineering/windows-native-recorder-roadmap.md).

## Upstream Contributions

The following work by this fork's maintainer was merged into the upstream OpenScreen repository. It
belongs to upstream, not to CloseScreen's own feature set:

- <https://github.com/getopenscreen/openscreen/pull/73> — software H.264 fallback in the WGC capture
  helper.
- <https://github.com/getopenscreen/openscreen/pull/152> — Notes teleprompter mode.
- <https://github.com/getopenscreen/openscreen/pull/228> — teleprompter read-only fix.

CloseScreen does not ship a Notes or teleprompter feature; those changes live upstream. Pull request
numbers in this repository are unrelated to upstream numbering and should not be read across.
...
- `.github/workflows/ci.yml` runs lint, i18n parity, typecheck, unit tests, browser tests, and Vite build on pull requests and `main`, plus a Rust lint job and a Rust build-and-test job on `windows-2022`.
