...
- PR evidence: `https://github.com/pjyqifei02/openscreen/pull/30` is open from `chore/demac-personalization` to `main`; PR CI checks Lint, Type Check, Test, and Build passed.
- Trimmed build workflow evidence: `https://github.com/pjyqifei02/openscreen/actions/runs/27371559635` concluded `success` on head `146cbbabb6503dcbedafa140df40c2fc61916329`.
...
- Migrated issue count: `gh issue list --repo pjyqifei02/openscreen --state all --label upstream-migration --limit 200 --json number --jq 'length'` returned `29`.
...
- Example migrated issue: `https://github.com/pjyqifei02/openscreen/issues/19` for upstream #602 starts with code-spanned source URL and `Source: ` code span.
...
- AC5: `build.yml` run `https://github.com/pjyqifei02/openscreen/actions/runs/27371559635` concluded `success` on the trimmed workflow branch and uploaded `windows-installer` size 386907134 plus `linux-installer` size 851792127.
