...
| --- | --- | --- |
| Windows 10 2004+ / Windows 11 | Primary maintained target; unsigned NSIS installer | `electron-builder.json5` `win.target`, `.github/workflows/build.yml`, `.github/workflows/release.yml`, `electron/native/wgc-capture` |
...
| --- | --- | --- | --- | --- |
| Screen recording | Supported primary path | Community-supported browser/Electron path | Not verified | Windows native helper uses WGC; Linux enables `WebRTCPipeWireCapturer` on Wayland. |
...
| Webcam overlay | Supported | Community-supported | Not verified | Renderer/export code supports webcam overlay; Windows helper has MediaFoundation/DirectShow webcam tests. |
...
| Update check | Supported as GitHub Releases check/link only | Supported as GitHub Releases check/link only | Not verified | Main-process `check-for-updates` reads `My-Denia/CloseScreen` GitHub Releases and returns a release URL; no auto-install provider is configured. |
...
| Low disk warning | Supported warning, not a hard block | Supported warning, not a hard block | Not verified | `electron/diskSpace.ts` checks free space and callers treat unknown as non-blocking. |
...
- Windows: `npm run build:native:win`, `npm run test:wgc-full:win`, and a full MP4/GIF export from
  the UI.
